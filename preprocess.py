# copied from FYP04 notebook, to be corrected
...
# folder = "data/"
folder = "gdrive/MyDrive/FYP/Data/DRP/"
root_folder = "gdrive/MyDrive/FYP/Data/DRP/root_folder/"

def load_drug_list(): 
    '''
      Returns list of all drugs in Druglist.csv
    '''
...
def write_drug_cid():
    '''
      Listing out unknown drugs in a csv file
    '''
...
def smile_to_graph(smile):
    """
      Input : (str) smile
      Output : 
        (int) c_size : Number of atoms in drug 
        (list) features : List of arrays containing normalised values 
        (list) edge_index : List of lists. Two values in each nested list contining Node index from one atom to other atom
        (graph) g : Graph of the drug
    """
...
def load_drug_smile():
    """
      Output : 
        ...
        (dictionary) smile_graph : Keys - (str) smiles of all drugs, Values - (tup) four outputs of function smile_to_graph(smile)
    """
...
def save_mix_drug_cell_matrix():
    ...
    for data in temp_data:
        ...
        if drug in drug_dict and cell in cell_dict:
            ...
            xc.append(cell_feature[cell_dict[cell]])      ## appending numpy array of shape (len(mut_dict),) ie. (735,) to list xc.  This is the cell_feature.  appending array of all genetic_feature
